...
describe("normalizeAgentDefaultsForJoin (openclaw_gateway)", () => {
  ...
  it("allows routing strategy and passes through valid sessionKeyRouting rules", () => {
    const normalized = normalizeAgentDefaultsForJoin({
      adapterType: "openclaw_gateway",
      defaultsPayload: {
        url: "ws://127.0.0.1:18789",
    ...
    expect(normalized.fatalErrors).toEqual([]);
    expect(normalized.normalized?.sessionKeyStrategy).toBe("routing");
    expect(normalized.normalized?.sessionKeyRouting).toEqual([
      { pattern: "assignment:*", sessionKey: "route:{{issueId}}" },
    ]);
  });
});
